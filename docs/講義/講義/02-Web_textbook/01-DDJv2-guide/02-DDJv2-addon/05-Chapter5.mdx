...
尋找事件名稱請參閱[附錄：事件相關附錄](../Appendix/#事件相關附錄)
...
可以參閱[附錄：Handler 相關附錄](../Appendix#handler-相關附錄)來查看各個路由所對應的 Handler 名稱
:::info[關於事件參數]
可以直接傳入
```ts
...
```
常用服務可參閱[附錄：常用服務列表](../Appendix#Service-服務相關附錄)
...
```
關於路由可參閱[附錄：Handler 相關附錄](../Appendix#handler-相關附錄)
範例
```ts
...
可以參閱[附錄：Handler 相關附錄](../Appendix#handler-相關附錄)來查看各個路由所對應的 Handler 名稱 
和[剛剛的小節](#handler/before\(after\))一樣的概念
