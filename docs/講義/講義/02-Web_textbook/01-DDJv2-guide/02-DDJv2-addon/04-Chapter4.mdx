...
# <span class="chapter_subtitle"> Addon Basics </span>
...
### II. Config 設定
我們可以在服務中定義一個 `config` 屬性來設定服務的相關參數 網站管理員可在後台(`/manage/config`)設定。
```typescript   
import {
    { Schema }
} from 'hydrooj';
...
- `.required()` 用來標記此參數為必要參數 (選配)。 // 用在必要參數上 若使用者未設定則會報錯
這樣我們就定義了一個服務的設定參數 `necessary` 和 `optional`
如下所示(此為`necessary`的範例)
<Img name="01-setting_sample.png" />

```typescript
test: Schema.string().description('請輸入描述'),
```
這樣也可以
會建議若無`.required()`則要加上`.default()`來避免參數為`undefined`的情況發生
...
```
這個方法會在服務初始化後被呼叫 其實和 `constructor` 一模一樣
因為 `apply` 方法是舊版的寫法 現在推薦直接使用 `constructor` 來進行初始化
也就是說 兩者擇一即可
:::
...
[第六章](../06-Chapter6.mdx)會有更詳細的 handler 介紹
...
[第五章](../05-Chapter5.mdx)會有更詳細的事件介紹
